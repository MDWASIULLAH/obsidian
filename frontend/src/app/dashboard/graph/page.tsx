...
const COLORS: Record<string, string> = {
  Repository: "#00e5ff", File: "#3b82f6", Dependency: "#ff7900", Vulnerability: "#ff3366",
  Threat: "#ec4899", Agent: "#00ff88", Fix: "#10b981", Scan: "#8b5cf6",
};
...
function project(n: VNode, width: number, height: number, time: number, zoom: number) {
  const ry = time * 0.00018;
  const rx = Math.sin(time * 0.00011) * 0.22;
...
function initialNodes(nodes: Node[], width: number, height: number): VNode[] {
  ...
  return nodes.map((n, i) => {
    ...
    const h = hash(n.id);
    const theta = (i / Math.max(nodes.length, 1)) * Math.PI * 2 + (h % 100) / 100;
    const phi = ((h % 180) * Math.PI) / 180;
    const r = radius * (0.55 + ((h % 100) / 100) * 0.8);
...
export default function GraphPage() {
  ...
  const [repos, setRepos] = useState<Array<{ id: string; full_name: string }>>([]);
  const [repoId, setRepoId] = useState("");
  const [graph, setGraph] = useState<Graph | null>(null);
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(true);
  const [hovered, setHovered] = useState<VNode | null>(null);
  ...
  const loadGraph = useCallback(async () => {
    ...
    try {
      ...
      const next: Graph = await res.json();
    ...
    } catch (e: any) { setError(e?.message || "Unable to load live graph"); }
  ...
  const draw = useCallback((time: number) => {
    ...
    const positions = new Map(nodes.map(n => [n.id, project(n, width, height, time, zoomRef.current)]));

    ctx.fillStyle = "#08090c"; ctx.fillRect(0, 0, width, height);
    const grad = ctx.createRadialGradient(width / 2, height / 2, 10, width / 2, height / 2, Math.max(width, height) * .65);
    grad.addColorStop(0, "rgba(20,35,45,.32)"); grad.addColorStop(1, "rgba(0,0,0,0)"); ctx.fillStyle = grad; ctx.fillRect(0, 0, width, height);
    ...
    for (const e of g.edges) {
      ...
      ctx.strokeStyle = hot ? `rgba(255,51,102,${0.28 + a.scale * .18})` : `rgba(65,170,190,${0.12 + a.scale * .12})`;
      ctx.lineWidth = hot ? 1.7 : 1; ctx.stroke();
      const p = ((time * 0.00008 + hash(e.source + e.target) / 10000) % 1);
      const px = a.x + (b.x - a.x) * p, py = a.y + (b.y - a.y) * p;
      ctx.fillStyle = hot ? "#ff3366" : "#45dbe8"; ctx.globalAlpha = .7;
      ctx.beginPath(); ctx.arc(px, py, hot ? 2 : 1.4, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = 1;
    }

    [...nodes].sort((a, b) => (positions.get(a.id)?.z || 0) - (positions.get(b.id)?.z || 0)).forEach(n => {
      const p = positions.get(n.id)!;
      const pulse = 1 + Math.sin(time * 0.004 + n.phase) * 0.12;
      const base = n.type === "Repository" ? 15 : n.type === "Vulnerability" ? 9 : 7;
      const r = Math.max(3, base * p.scale * pulse);
      const c = color(n.type);
      ctx.shadowBlur = 20 * p.scale; ctx.shadowColor = c;
      const glow = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, r * 3.5);
      glow.addColorStop(0, `${c}88`); glow.addColorStop(1, `${c}00`); ctx.fillStyle = glow;
      ctx.beginPath(); ctx.arc(p.x, p.y, r * 3.5, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0;
      ...
      ctx.font = `${Math.max(9, Math.round(11 * p.scale))}px Inter, sans-serif`; ctx.fillStyle = "#d5dde5"; ctx.textAlign = "center";
      const label = n.label.length > 28 ? `${n.label.slice(0, 26)}…` : n.label; ctx.fillText(label, p.x, p.y + r + 14);
    });
    frameRef.current = requestAnimationFrame(draw);
  }, []);

  useEffect(() => { frameRef.current = requestAnimationFrame(draw); return () => { if (frameRef.current) cancelAnimationFrame(frameRef.current); }; }, [draw]);

  const onPointer = (e: React.PointerEvent<HTMLCanvasElement>) => {
    const canvas = canvasRef.current; if (!canvas) return;
    const rect = canvas.getBoundingClientRect(); let best: VNode | null = null; let distance = 28;
    for (const n of nodesRef.current) { const p = project(n, rect.width, rect.height, performance.now(), zoomRef.current); const d = Math.hypot(p.x - (e.clientX - rect.left), p.y - (e.clientY - rect.top)); if (d < distance) { best = n; distance = d; } }
    setHovered(best);
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div><div className="flex items-center gap-3"><h1 className="text-xl font-bold text-gray-100">Knowledge Graph</h1><span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-full bg-cyan-400/10 text-cyan-300 text-[10px] font-semibold"><Activity className="w-3 h-3 animate-pulse" /> LIVE</span></div><p className="text-sm text-gray-500 mt-1">3D repository intelligence — real dependencies, files, vulnerabilities, threats, agents and fixes.</p></div>
        <div className="flex items-center gap-2">{repos.length > 0 && <select value={repoId} onChange={e => setRepoId(e.target.value)} className="bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-xs text-gray-300 max-w-[230px]">{repos.map(r => <option key={r.id} value={r.id}>{r.full_name}</option>)}</select>}<button onClick={loadGraph} className="p-2 rounded-lg glass-card"><RefreshCw className="w-4 h-4 text-gray-400" /></button><button onClick={() => zoom(.2)} className="p-2 rounded-lg glass-card"><ZoomIn className="w-4 h-4 text-gray-400" /></button><button onClick={() => zoom(-.2)} className="p-2 rounded-lg glass-card"><ZoomOut className="w-4 h-4 text-gray-400" /></button><button onClick={reset} className="p-2 rounded-lg glass-card"><RotateCcw className="w-4 h-4 text-gray-400" /></button><button onClick={() => setFullscreen(v => !v)} className="p-2 rounded-lg glass-card"><Maximize2 className="w-4 h-4 text-gray-400" /></button></div>
      </div>
