...
import { Shield, LayoutDashboard, GitBranch, Search, Bot, Network, FileWarning, FileText, Settings, Zap, Cpu, TrendingUp, Swords, DollarSign, History, Menu, X, LogOut } from "lucide-react";
...
function Sidebar({ pathname, session, mobile = false, onClose, backendOnline }: { pathname: string; session: any; mobile?: boolean; onClose?: () => void; backendOnline: boolean }) {
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    let cancelled = false;
    const check = async () => {
      try { const response = await fetch(`${API_BASE}/health`, { cache: "no-store" }); if (!cancelled) setBackendOnline(response.ok); }
  ...
  return <div className="flex h-[100dvh] min-h-0 overflow-hidden bg-surface-950">
    <Sidebar pathname={pathname} session={session} backendOnline={backendOnline} />
